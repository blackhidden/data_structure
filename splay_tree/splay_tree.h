...
template<typename T>
inline Splay_tree<T>::Splay_tree() : hb_root(0), hb_size(0) {
	if (null_node == 0) {
...
template<typename T>
inline Splay_tree<T>::~Splay_tree() {
	clear();
}
...
template<typename T>
inline void Splay_tree<T>::destroy(PBtn pTn) {
	if (null_node == pTn)
		return;
...
template<typename T>
inline int Splay_tree<T>::depth(const PBtn pTn) const {
	if (null_node == pTn)
		return 0;
	else {
...
template<typename T>
inline typename Splay_tree<T>::PBtn Splay_tree<T>::find_min(PBtn pTn) const {
	if (pTn == 0)
		pTn = hb_root;
	if (pTn == null_node)
		return 0;
...
template<typename T>
inline typename Splay_tree<T>::PBtn Splay_tree<T>::find_max(PBtn pTn) const {
	if (pTn == 0)
		pTn = hb_root;
	if (pTn == null_node)
		return 0;
...
template<typename T>
inline typename Splay_tree<T>::PBtn Splay_tree<T>::find(const T& elem) const {
	PBtn pTn = hb_root;
	while (pTn != null_node) {
		if (elem == pTn->element)
			return pTn;
...
template<typename T>
inline typename Splay_tree<T>::PBtn Splay_tree<T>::single_rotate_with_left(PBtn pTn) {
	PBtn ptm = pTn->lchild;
...
template<typename T>
inline typename Splay_tree<T>::PBtn Splay_tree<T>::single_rotate_with_right(PBtn pTn) {
	PBtn ptm = pTn->rchild;
...
template<typename T>
inline typename Splay_tree<T>::PBtn Splay_tree<T>::insert(const T& elem, PBtn pTn) {
...
template<typename T>
inline typename Splay_tree<T>::PBtn Splay_tree<T>::erase(const T& elem, PBtn pTn) {
	if (pTn == null_node) {
		std::cerr << "error, the splay tree if empty!\n";
		return pTn;
	}
...
template<typename T>
inline typename Splay_tree<T>::PBtn Splay_tree<T>::splay(const T& elem, PBtn pTn) {
...
template<typename T>
inline void Splay_tree<T>::print_tree(std::ostream& output, PBtn pTn, int depth) const {
	if (pTn == null_node)
		return;
