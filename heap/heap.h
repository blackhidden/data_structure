...
class Heap {
public:
	explicit Heap(int mx_size, const T limit);
	~Heap();
...
template<typename T>
inline Heap<T>::Heap(int max_size, const T limit) : hb_capacity(max_size), hb_size(0) {
...
template<typename T>
inline void Heap<T>::print(std::ostream& output) const {
	if (empty()) return;
	output << "-------------------------------------\n";
	int i = 1;
	while (true) {
