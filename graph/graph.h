...
public:
	...
	virtual void visit(T_vertex ver, void (*client_visit)(T_vertex ver) = 0);
...
template<typename T_vertex, typename T_edge>
inline Graph_matrix<T_vertex, T_edge>::Graph_matrix(int max_num, T_edge infinity, T_vertex novert) : hb_num_vert(0), hb_num_edge(0), hb_max_num(max_num), hb_infinity(infinity), hb_novert(novert)
{
...
template<typename T_vertex, typename T_edge>
inline Graph_matrix<T_vertex, T_edge>::~Graph_matrix() {
	delete[] vertex;
	for (int i = 0; i != hb_max_num; ++i) {
		delete[] adj_matrix[i];
	}
...
template<typename T_vertex, typename T_edge>
inline void Graph_matrix<T_vertex, T_edge>::visit(T_vertex ver, void (*client_visit)(T_vertex ver)) {
	if (client_visit == 0)
		std::cerr << ver << ' ';
	else 
		client_visit(ver);
...
template<typename T_vertex, typename T_edge>
inline int Graph_matrix<T_vertex, T_edge>::find(T_vertex ver) const {
	for (int i = 0; i != hb_max_num; ++i) {
		if (vertex[i] == ver)
			return i;
	}
