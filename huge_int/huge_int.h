...
inline Huge_int::Huge_int(int val) {
	if (val >= 0)
		sign = true;
	else {
		sign = false;
...
inline Huge_int::Huge_int(string& str) {
	sign = true;
...
inline void Huge_int::Huge_int(const Huge_int& rhs) {
	sign = rhs.sign;
	digits = rhs.digits;
}

inline Huge_int& Huge_int::operator=(const Huge_int& rhs) {
	digits = rhs.digits;
	sign = rhs.sign;
	return (*this);
}

inline Huge_int Huge_int::abs() const {
	if (sign)
		return *this;
	else
		return -*this;
...
inline Huge_int Huge_int::pow(int n) {
	Huge_int res(1);
	if (n == 0) return res;
	if (n == 1) return *this;
	if (n % 2 == 0) {
...
inline Huge_int& Huge_int::operator+=(const Huge_int& rhs) {
	if (sign == rhs.sign) {
		...
		vector<char>::iterator iter2 = rhs.digits.begin();
...
inline Huge_int& Huge_int::opeartor*=(const Huge_int& rhs) {
	Huge_int result(0);
