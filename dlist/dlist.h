...
template<typename T>
inline Dlist<T>::Dlist() : hb_head(new Dlist_node<T>), hb_tail(new Dlist_node<T>), hb_size(0) {
...
template<typename T>
inline Dlist_node<T>* Dlist<T>::find(int pos) const {
...
template<typename T>
inline void Dlist<T>::insert_after(const T& elem, int pos) {
	Ptrn pbef = find(pos);
	if (pbef == 0) return;
	Ptrn paft = pbef->next;
...
template<typename T>
inline void Dlist<T>::push_back(const T& elem) {
	insert_after(elem, hb_size);
}

template<typename T>
inline void Dlist<T>::push_front(const T& elem) {
	insert_after(elem, 0);
}

template<typename T>
inline T Dlist<T>::erase(Dlist_node<T>* pSln) {
...
template<typename T>
inline T Dlist<T>::erase(int pos) {
	Ptrn pSln = find(pos);
	assert(pSln != 0);
	return erase(pSln);
}
...
template<typename T>
inline T Dlist<T>::pop_front() {
	return erase(1);
}

template<typename T>
inline T Dlist<T>::pop_back() {
	return erase(hb_size);
}

template<typename T>
inline void Dlist<T>::erase(int beg, int end) {
	Ptrn pbeg = find(beg);
	Ptrn pend = find(end);
	if (pbeg == 0 || pend == 0) return;
...
template<typename T>
inline T Dlist<T>::value(int pos) const {
	Ptrn pSln = find(pos);
	assert(pSln != 0);
	return pSln->element;
}
